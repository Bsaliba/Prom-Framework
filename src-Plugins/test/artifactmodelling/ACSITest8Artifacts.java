...
public class ACSITest8Artifacts {
	...
	@Plugin(name = "ACSI CD Shop (8 artifacts)", parameterLabels = {}, returnLabels = "Artifact Model", returnTypes = ArtifactModel.class, userAccessible = true)
	public ArtifactModel createArtifactModel(PluginContext context) {
		...
		Proclet custShip = getCustShip();
		Proclet custNotif = getCustNotif();

		ArtifactModel model = new ArtifactModelImpl("CD Shop (8 artifacts)");
		...
		model.addArc((Output) ports[6][2], (Input) ports[5][0])
				.setLabel("Mail");

		model.addArc((Output) ports[3][1], (Input) ports[2][0]).setLabel(
				"E-Bank");
	...
	Proclet getCDShopShip() {
		...
		ProcletTransition t3 = p.addTransition("Ship items");
		...
		ports[6][2] = p.addPort(Cardinality.ONE, Multiplicity.ONE, true);

		p.addReceiveArc(ports[6][0], t1);
		p.addSendArc(t2, ports[6][1]);
		p.addSendArc(t3, ports[6][2]);
	...
	Proclet getCustShip() {
		...
		ProcletTransition t1 = p.addTransition("Create CD shop shipment");
		ProcletTransition t2 = p.addTransition("Ship quote to the customer");
		ProcletTransition t3 = p.addTransition("Finish CD shop shipment");
		...
		ports[5][0] = p.addPort(Cardinality.ONE, Multiplicity.ONE, false);

		p.addReceiveArc(ports[5][0], t1);
	...
	Proclet getCDShopNotif() {
		...
		ports[3][0] = p.addPort(Cardinality.AT_LEAST_ONE, Multiplicity.ONE,
				false);
		ports[3][1] = p.addPort(Cardinality.ONE, Multiplicity.ONE, true);

		p.addReceiveArc(ports[3][0], t1);
		p.addSendArc(t2, ports[3][1]);
		return p;
	}

	Proclet getCustNotif() {
		Proclet p = new ProcletImpl("Customer Notification");

		ProcletTransition t1 = p.addTransition("Create customer notification");
		ProcletTransition t2 = p.addTransition("Send notification");
		ProcletTransition t3 = p.addTransition("Finish customer notification");
		...
		ports[2][0] = p.addPort(Cardinality.ONE, Multiplicity.ONE, false);

		p.addReceiveArc(ports[2][0], t1);
	...
	Proclet getOrder() {
		...
		ProcletTransition t2 = order.addTransition("Add to order");
		ProcletTransition t3 = order.addTransition("Add to order");
		ProcletTransition t4 = order.addTransition("Ship to CDShop");
		ProcletTransition t5 = order.addTransition("invisible");
		ProcletTransition t6 = order.addTransition("Generate invoice");
		ProcletTransition t7 = order.addTransition("Notify undel.");
		ProcletTransition t8 = order.addTransition("invisible");
		ProcletTransition t9 = order.addTransition("Destroy order");
